...
Библиотека matplotlib-cpp представляет собой реализацию на языке C++ удобного и широко распространённого подхода к построению графиков. Преимущественно она использует средства, используемые в matplotlib - пожалуй, одном из наиболее популярных инструментов API для построения графиков.
...
В данном репозитории представлена дополненная версия matplotlib-cpp, которая прежде всего устраняет все неудобства, связанные с построением 3D графиков, а также содержит альтернативные методы создания графиков, основанные на несколько иных принципах, чем те, что использовались в оригинальной библиотеке. Представляемая версия также содержит ряд исправлений, устраняющих вышеупомянутые несовместимости со средой разработки и ОС.

## Предварительные замечания

Ниже мы предлагаем инструкцию подключения библиотеки maplotlib-cpp, которая может быть применена для:

- IDE: **Microsoft Visual Studio**
- ОС: **Microsoft Windows**
- Версия Python: **Python 3.9**

Гарантированная работа библиотеки при её правильном подключении подразумевает использование именно Python 3.9, а не более поздних релизов, которые уже могут быть несовместимы с matplotlib-cpp. Версию Python 3.9 можно скачать с официального сайта разработчика.

## Инструкция подключения

1) Скачайте заголовочный файл `matplotlibcpp.h`, расположенный в папке `MyLib` данного репозитория. Подключите его к вашему проекту стандартным способом.
...
Вид ваших путей зависит от папки, куда был установлен Python 3.9, однако рекомендуется выбрать путь установки аналогичный тому, что был представлен выше (название папки user может быть другим).
4) Затем в тех же настройках выберите "Каталоги библиотек" и укажите путь к оставшимся используемым библиотекам Python. Наш путь выглядит так:
    - `"C:\Users\user\AppData\Local\Programs\Python\Python39\libs"`.
5) Перезапустите ваш проект.
...
Здесь мы приведём синтаксис и описание методов, которые были добавлены нами для успешного построения 3D графиков, а также их аналогов для 2D случая, которые работают по тому же принципу.

- `chart(int i)` \
Создаёт объект, содержащий графическое поле.

Для построения одиночного графика необходимо писать:
```cpp
PyObject* ax = matplotlibcpp::chart(111);
```
\
Также этот метод позволяет строить подграфики и тем самым он заменяет оригинальный метод subplot. Он принимает в качестве аргумента натуральное трёхзначное число, разряды которого задают следующие параметры: первый разряд - количество строк для подграфиков в общем графическом поле, второй - количество столбцов, третий - порядковый номер подграфика в массиве подграфиков и его место в общем графическом поле, разделённом на строки и столбцы. Например:
```cpp
std::vector<PyObject*> plots;
for(auto & place : {221, 222, 223, 224}){
    plots.push_back(matplotlibcpp::chart(place));
}
```
...
Таким образом, применяемый для создания одиночного графика метод `matplotlibcpp::chart(111)` на самом деле создаёт подграфик, который занимает всё общее графическое поле. Благодаря такому подходу можно легко переоринтировать графическое поле под множество подграфиков или под одиночный график, просто изменив аргумент метода `chart` соответствующим образом.
...
Устанавливает диапазон отображаемых значений на оси Z. В отличие от других осей, для оси Z в аргументах указывается объект класса PyObject, содержащий графическое поле.
...
Для создания простых 2D графиков можно использовать как предложенные нами, так и оригинальные методы.
...
![subplots example](./MyLib/subplots.png)
---
